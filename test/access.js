Test("tracking property access", (api, pass, fail, assert) => {
	...
	let fooList = api.access.trackedValues(tracker.foo);
	assert(fooList instanceof Set);
	assert(fooList.size == 2);
	assert(fooList.has('tracked1'));
	assert(fooList.has('tracked2'));

	let bazList = api.access.trackedValues(tracker.baz);
	assert(!bazList || bazList.size == 0);

	let baz0List = api.access.trackedValues(tracker.baz[0]);
...
Test("piercing gets original object", (api, pass, fail, assert) => {
	...
	let trackedList = api.access.trackedValues(tracker);
